      ...
      >
        <div class="box-default q-pa-xl text-center form-card">
          ...
          <q-input
            ...
            @change="updateValue('carbohydrates')"
          />
          <q-input
            outlined
            dense
            ...
            @change="updateValue('energy_value')"
          />
          <q-input
            outlined
            dense
...
export default defineComponent({
  ...
  async created() {
    ...
    const object = (await FoodService.show(this.id)).data
    object.food_category = object.food_category.id
  ...
  methods: {
    ...
    async save() {
      if (await this.$confirmation('save')) {
        try {
          await FoodService.put(this.id, this.object)
          this.$router.back()
        } catch (err) {
          console.log(err)
        }
