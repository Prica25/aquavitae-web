...
export default defineComponent({
  ...
  async created() {
    const object = (await BiochemicalDataService.show(this.id)).data
    for (const key in this.object) {
      this.object[key] = object[key]
    }
    this.object.appointment = object.appointment.id
    ...
    async save() {
      if (await this.$confirmation('save')) {
        try {
          ...
          await BiochemicalDataService.put(this.id, this.object)
          this.$router.back()
        } catch (err) {
          console.log(err)
        }
