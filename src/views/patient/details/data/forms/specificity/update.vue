...
export default defineComponent({
  ...
  async created() {
    const object = (await SpecificityService.show(this.id)).data
    this.object.food = object.food.id
    this.object.specificity_type = object.specificity_type.id
  },
  methods: {
    async goBack() {
    ...
    async save() {
      if (await this.$confirmation('save')) {
        try {
          await SpecificityService.put(this.id, this.object)
          this.$router.back()
        } catch (err) {
          console.log(err)
        }
